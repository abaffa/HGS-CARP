...
void Params::getClient(int i, Client * myCli)
{
	...
	if (type == 30 || type == 33) // CARP instances (also used when running MDCARP experiments, which only involve to define more depots)
	{
		...
		else
		{
			...
			if (!directedGraph) {
				myCli->ar_serviceCost10 = myCli->ar_serviceCost01; //UNDIRECTED CARP

				// setting the distance between the nodes (here its symmetric)
				ar_distanceNodes[myCli->ar_nodesExtr1][myCli->ar_nodesExtr0] = myCli->ar_serviceCost01; //UNDIRECTED CARP
			}
...
void Params::tspLikeLoader_getClient(int i, Edge edge, Client * myCli)
{
	// Reading/Initializing the data for each customer
	string tempstring;
	pattern p;
	...
	if (type == 30 || type == 33) // CARP instances (also used when running MDCARP experiments, which only involve to define more depots)
	{
		...
		{
			myCli->ar_nodesExtr0 = edge.from;
			myCli->ar_nodesExtr1 = edge.to;
			myCli->ar_serviceCost01 = edge.cost;
			myCli->ar_serviceCost10 = edge.cost;

			// setting the distance between the nodes (here its symmetric)
			ar_distanceNodes[myCli->ar_nodesExtr0][myCli->ar_nodesExtr1] = myCli->ar_serviceCost01;
			ar_distanceNodes[myCli->ar_nodesExtr1][myCli->ar_nodesExtr0] = myCli->ar_serviceCost01;

			myCli->demand = edge.demand;
			totalDemand += myCli->demand;
			myCli->ar_nodeType = AR_CLIENT_EDGE;
		}
	}
...
Params::Params(string nomInstance, string nomSolution, string nomBKS, int seedRNG, int type, int format, bool directed, int nbVeh, int nbDep, bool isSearchingFeasible) :type(type), format(format), nbVehiculesPerDep(nbVeh), nbDepots(nbDep), isSearchingFeasible(isSearchingFeasible)
{
	...
	if (format == 0) {
		...
		TspLikeLoader loader;
		loader.load(nomInstance);

		double vc = loader.car_capacity;
		ar_NodesNonRequired = loader.n_nodes;
		ar_EdgesRequired = loader.n_edgesRequired;
		ar_EdgesNonRequired = loader.n_edgesNonRequired;

		//"m = 1 + 1.2 \times m_LB" vehicles, this gives one extra vehicle plus 20 % , and should be largely sufficient in all cases.
		nbVehiculesPerDep = 1 + (1.2 * loader.car_number);

		//vc = loader.car_capacity;
		ar_NodesRequired = 0;
		ar_ArcsRequired = 0;
		ar_ArcsNonRequired = 0;
		...
		tspLikeLoader_getClient(i, e, &cli[i]);
		for (int j = 0; j < loader.edges.size(); j++) {
			e = loader.edges[j];
		...
		tspLikeLoader_ar_parseOtherLinesCARP(&loader);

		if (loader.depot_indexes.size() == 0) {
			cli[0].ar_nodesExtr0 = 0;
			cli[0].ar_nodesExtr1 = 0;
		}
		else
		{
			cli[0].ar_nodesExtr0 = loader.depot_indexes[0];
			cli[0].ar_nodesExtr1 = loader.depot_indexes[0];
		}

		//ar_computeDistancesNodes(); // We only need the distance between endpoints
		tspLikeLoader_ar_computeDistancesNodes();
	}
	else
...
void Params::tspLikeLoader_ar_computeDistancesNodes()
{
...
void Params::tspLikeLoader_ar_parseOtherLinesCARP(TspLikeLoader *loader)
{
	// Parsing routine for CARP
	string contenu;
	string useless;
